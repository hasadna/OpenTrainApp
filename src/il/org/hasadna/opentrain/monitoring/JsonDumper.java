...
public class JsonDumper {
	...
	public static File getLogsDir(Context context) {
		if (isExternalStorageMounted()) {
			...
			Log.d(LOGTAG,
					"createExternalStoragePrivateFile: dir=" + dir.toString());
			return dir;
		} else {
	...
	public void dump(String rawDataType, JSONArray rawJson) {
		if (!mIsInitialized) {
			...
			try {
				...
				timestampedJson.put("date",
						DateTimeUtils.formatTimeForLocale(timestamp));

				timestampedJson.put(rawDataType, rawJson);
				stringToWrite = timestampedJson.toString(2); // toString(2) formats the json with 2 space indentation
			...
			Write(stringToWrite != null ? "{\"timestamp\"=" + stringToWrite
	...
	private void Write(String string) {
		try {
			...
			Log.w(LOGTAG_PER_DUMPER, "Write: log JSONArray:", e);
			e.printStackTrace();
		}
	}
	...
	void createExternalStorageFile(String fileName) {
		...
		if (path != null && path.mkdir() || path.isDirectory()) {
			...
			Log.d(LOGTAG_PER_DUMPER, "createExternalStoragePrivateFile: file="
